...
def build_mixture_loglogistic_model( dropout_rate = 0.1, ridge_penalty = 0.01, lasso_penalty = 0.01 ):
    def logit(u):
        return -( tf.math.log(1-u) - tf.math.log(u) )

    mixture_loglogistic_parameters = {
    ...
    def loglikelihood_loss(model, nn_output, data):
        ...
        log_f0 = log_beta + (beta-1)*log_y - beta*tf.math.log(alpha) - 2*log_term
        S0 = tf.math.exp(-log_term)
    ...
    return mixture_loglogistic_parameters, loglikelihood_loss, neural_network, neural_network_call, neural_network_call_nolast


with tf.device("/GPU:0"):
    mixture_loglogistic_parameters, mixture_loglogistic_loss, mixture_loglogistic_neural_network, mixture_loglogistic_call, mixture_loglogistic_call_nolast = \
    build_mixture_loglogistic_model( ridge_penalty = 0.001, lasso_penalty = 0.001 )
    seed = 10
    mixture_loglogistic_model = thf.ModelNN(mixture_loglogistic_parameters, mixture_loglogistic_loss,
                                        mixture_loglogistic_neural_network, mixture_loglogistic_call,
                                        mixture_loglogistic_call_nolast, input_dim = (n_features,), seed = seed)
    mixture_loglogistic_model.pre_train_model(epochs = None, x = train_ds, data = None, n_train = n_train, shuffle = True)
    mixture_loglogistic_model.train_model(epochs = 1500, x = train_ds, data = None, n_train = n_train,
                                      shuffle = True,
...
mixture_loglogistic_model.hessian_jitter = 1.0e-6
mixture_loglogistic_model.save_model("mixture_loglogistic_model")